...
int main() {
  ...
  // int intervels[7][2] = {{0, 2}, {1, 4},  {4, 6}, {0, 4}, {7, 8}, {9, 11},
  // {3, 10}};
  ...
  for (int i = 0; i < intervel_size; i++) {
    if (size >= 0 && laptop_heap[0] <= intervels[i][0]) {
      deleteRoot(laptop_heap, laptop_heap[0]);
    }
    insert(laptop_heap, intervels[i][1]);

    printf("dbg: 2");
    laptop_needed = laptop_needed < size ? size : laptop_needed;
  }

  printArray(laptop_heap, size);

  printf("laptop needed: %d\n", size);
}
